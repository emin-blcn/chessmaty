...
func update_white_time_gui():
	...
	if config_data["time_per_side"] >= 3_600_000:
		white_time_label.text = "%02d:%02d:%02d" % [hour, minute, second]
	elif config_data["time_per_side"] >= 60_000:
...
func update_black_time_gui():
	var black_time_label: Label = top_time_label if config_data["player_color"] == Enums.ChessColor.WHITE else bottom_time_label
	@warning_ignore_start("integer_division")
